...
socket.on('positions', (updatedPositions: Position[], latestPrice: number) => {
  const unresolvedPositions = updatedPositions.map((position) => ({
    ...position,
  ...
  socket.on('priceUpdate', (price: number) => {
    ...
    setPreviousPrice(isNaN(price) ? (previousPrice || 0) : price);
  });
        ...
        {unresolvedPositions.map((item) => {
          ...
          return (
            <tr key={item._id}>
              ...
              <a href={`https://solscan.io/account/${item.binaryOption}?cluster=devnet`} target="_blank" rel="noreferrer">
    <p>{`${item.binaryOption.slice(0, 6)}...${item.binaryOption.slice(-6)}`}</p>
  </a>
              </td>
              <td className="pb-4">
