...
if (takeProfitPrice != 0) {
  ...
  createPositionLine(formattedPrice, 'Take Profit', '#34C796');
        ...
        useEffect(() => {
          ...
          console.log("Mapped index:", index, "Position data:", position);
        
          removePositionLines();
        
          if (position) {
            console.log("Updating chart lines for symbol:", symbol);
            updateChartLines(position);
          } else {
            console.log("No position data for new symbol:", symbol);
          }
        }, [symbol]);
        
        // ... other useEffect for latestOpenedPosition ...
