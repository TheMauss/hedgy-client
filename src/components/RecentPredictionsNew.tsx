...
const RecentPredictions: FC<Recentprops> = ({ divHeight }) => {
  ...
  return (
    <div className="font-poppins w-full flex flex-col overflow-hidden h-full md:overflow-y-scroll  custom-scrollbar  order-5 xl:order-3 lg:order-3 md:order-5 rounded-lg bg-layer-1 p-4  flex bg-layer-1">
      <div className="sticky top-0 flex items-center z-10">
        <h2 className="bankGothic leading-[18px] text-xl text-start text-grey-text">
          Recent Predictions
        </h2>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="custom-scrollbar flex flex-col overflow-auto ">
          <div className="sticky top-0 bg-layer-1 flex z-10 pt-2">
            <div className="w-[25%] text-sm leading-[12px] text-grey-text py-2 ">
              User
            </div>
            ...
            <div className="w-[17%] text-sm leading-[12px] text-grey-text py-2">
              ROI
            </div>
          </div>
          <FlipMove>
            {Array.isArray(positions) &&
              positions.map((item, i) => {
                ...
                return (
                  <div
                    key={item._id}
                    className={`shadow-lg flex text-[0.9rem] rounded my-1.5  ${profit > 4.99 && profit > 0 ? "text-slate-300" : "text-slate-300"}`}
                  >
                    ...
                    <div className="w-[25%] flex items-center text-white">
                      {"binaryOption" in item ? "BIN" : `${item.leverage}X`}
                    </div>
                    <div
                      className={`w-[25%] flex items-center text-right ${value < 0 ? "negative text-red-500" : "text-[#34c796]"}`}
                    >
                      {value.toFixed(2)}
                    </div>
                    <div
                      className={`w-[17%] flex items-center text-right pr-2 ${percentage < 0 ? "negative text-red-500" : "text-[#34c796]"}`}
                    >
