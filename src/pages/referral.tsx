...
import useUserSOLBalanceStore from '../../src/stores/useUserSOLBalanceStore';
  ...
  async function getAffiliateData(affiliatePublicKey: PublicKey, connection: Connection): Promise<{ account: AffiliateAccount }> {
...
const Referral: FC = () => {
    ...
    const onClick = useCallback(async () => {// Create the instruction to initialize the user account
        if (!publicKey) {
        ...
        if (userAffiliateData && !userAffiliateData.isInt) {
            try {
                ...
                notify({ type: 'success', message: `Trading account successfully created, now create the Referral.` });
          ...
          const [AffilAcc] = await PublicKey.findProgramAddress(
            ...
            // Create a new transaction to initialize the user account and send it
            const initTransaction = new Transaction().add(initializeAffilAcc(args, accounts));
            
            try {
                const initSignature = await sendTransaction(initTransaction, connection);
                    // Notify user that the transaction was sent
    notify({ type: 'info', message: `Trying to create the referral...`, txid: initSignature });
                // Wait for transaction confirmation
                await connection.confirmTransaction(initSignature, 'confirmed');
                setUserAffiliateData(prevState => ({ ...prevState, hasCode: true }));
                setUserAffiliateData(prevState => ({ ...prevState, isInt: true }));
                setUserAffiliateData(prevState => ({ ...prevState, myAffiliate: affiliateCodeToUint8Array(affiliateCode) }));
                notify({ type: 'success', message: `Your referral code has been created.`, txid: initSignature });
    ...
    }}}, [userAffiliateData, affiliateCode, publicKey, connection, sendTransaction, notify]);
...
useEffect(() => {
    if (userAffiliateData?.myAffiliate && userAffiliateData.myAffiliate.length > 0) {
        ...
        const fetchAffiliateData = async () => {
            ...
            const [affiliatePublicKey] = await PublicKey.findProgramAddress(seedsAffil, PROGRAM_ID);
            try {
...
const onClick1 = useCallback(async () => {// Create the instruction to initialize the user account
    if (affiliateData.totalEarned <= (0.1 * LAMPORTS_PER_SOL))
    {
        notify({ type: 'error', message: `Rewards are less than 0.1 SOL`, description: "Try again later." });
        return;
    ...
    if (!publicKey) {
      ...
      const [AffilAcc] = await PublicKey.findProgramAddress(
        ...
        // Create a new transaction to initialize the user account and send it
        const initTransaction = new Transaction().add(withdrawAffiliateEarnings(accounts));
...
notify({ type: 'info', message: `Trying to create the referral...`, txid: initSignature });
    ...
    }, [userAffiliateData, publicKey, connection, sendTransaction, notify, affiliateData]);
  ...
  const copyToClipboard = () => {
    if (decodedString) { // Check if decodedString is not empty
                ...
                {!userAffiliateData?.hasCode ? (
                    <div className="pt-4 flex md:flex-row flex-col items-center justify-center gap-[16px] text-xl text-white px-2 md:px-0 z-100">
                      ...
                      <div className="w-full h-full self-stretch md:rounded-2xl rounded-lg bg-gradient-to-t from-[#0B7A55] to-[#0b111b]  w-full flex flex-col items-start justify-center">
                        ...
                        Refer friends to trade on PopFi and earn a share of their trading fees!                        </div>
                      </div></div></div>
                ...
                <div className="z-10 md:w-[35%] self-stretch bg-gradient-to-t from-[#0B7A55] to-[#34C796] md:rounded-2xl rounded-lg p-[1px]">
                  ...
                  <div className="flex flex-col items-center justify-center text-right text-13xl text-white font-poppins">
                    <div className="flex flex-row items-center  justify-center gap-[8px]">
                      <div 
                      className="text-3xl relative leading-[100%] font-medium">{decodedString}</div>
                      <button>
                      <img
                  ...
                  <div className="font-poppins w-[287px] flex flex-col items-start justify-center gap-[32px]">
                    ...
                    <div className="flex flex-row items-center justify-start gap-[12px]">
                      ...
                      <div className=" flex flex-col items-start justify-center gap-[4px]">
                        ...
                        {(affiliateData?.totalEarned/LAMPORTS_PER_SOL).toFixed(2)} SOL
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col items-start justify-center gap-[32px]">
                    <div className="flex flex-row items-center justify-start gap-[12px]">
                      ...
                      <div className="flex flex-col items-start justify-center gap-[4px]">
                        ...
                        <div className="relative text-xl leading-[100%] font-medium font-poppins text-white text-right">
                        {(affiliateData?.totalAffiliatesVolume/LAMPORTS_PER_SOL).toFixed(1)}
                        </div>
                      </div>
                    </div>
